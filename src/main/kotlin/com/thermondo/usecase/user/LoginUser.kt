 ...
 */
class LoginUser(private val userRepository: IUserRepository) : UseCase<LoginUser.RequestModel, LoginUser.ResultModel>() {
    ...
    override fun execute(input: RequestModel): ResultModel {
        try {
            ...
            return ResultModel.successResult(existingUser.id.toString(), existingUser.userName.toString())
        } catch (e: Exception) {
    ...
    class ResultModel(val userId: String?, val userName: String?, override val successful: Boolean, override val message: String) : UseCase.ResultModel {
        ...
        companion object FactoryMethods {
            fun successResult(userId: String, userName: String) : ResultModel {
                return ResultModel(userId, userName, true, "Successfully logged in user '$userName'")
            }
            ...
            fun errorResult(userName: String?, errorReason: String?) : ResultModel {
                ...
                return ResultModel(null, effectiveUserName,false, "Unable to login user '$effectiveUserName' because of error: $effectiveErrorReason")
            }
        }
    }
}
